...
auto main() -> int
{
    lgx::Log(lgx::Level::Info, "Nature's abomination sweat milk.");
    lgx::Log(lgx::Level::Warn, "Sleep ddeprivation is bad.");
...
auto main() -> int
{
    ...
    const auto engine_logger = lgx::Logger{lgx::Logger::Properties { .defaultPrefix = "Engine", .format = "({datetime})-({prefix})-({level}): {msg}\n" }};
    engine_logger.Log(lgx::Level::Info, "Engine logger");
    return 0;
}
#+end_src
